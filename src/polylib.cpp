 ...
 * polylib.cpp
 *
 * Author: Spencer Sherwin. Modified by Xiaoliang Wan
 * 
 */
...
/*
 ...
 Based on the codes by Spencer Sherwin, Einar Ronquist and Ron Henderson
...
static double gammaF2(double, double);
...
void zwgj(double *z, double *w, int np, double alpha, double beta) 
{
  ...
  if(alpha == -0.5 && beta == -0.5)
    {
      for(i = 0; i < np; i++)
	z[i] = -cos((2.0*(double)i+1.0)*M_PI/(2.0*(double)(np-1)+2.0));
      for(i = 0; i < np; i++)
	w[i] = M_PI/(double)np;
      return;
    }
  ...
  //fac = pow(two, apb + one) * gammaF(alpha + np + one) * gammaF(beta + np
  //								+ one);
  //fac /= gammaF(np + one) * gammaF(apb + np + one);

  fac  = pow(two, apb + one);
  fac *= gammaF2(alpha + np + one, np + one);
  fac *= gammaF2(beta  + np + one, np + one + apb);
...
/**
 ...
 Generate \a np Gauss-Radau-Jacobi zeros, z, and weights,w,
...
{  
  if (np == 1) 
	...
	{
	  for(i = 0; i < np; i++)
	    z[i] = -cos(2.0*M_PI*(double)i/(double)(2*np-1));
	  for(i = 1; i < (np-1); i++)
	    w[i] = M_PI/(double)(2*np-1);
	  w[0] = w[np-1] = M_PI/2.0/(double)(2*np-1);
	  return;
	}
      ...
      fac  = pow(two, apb);
      fac *= gammaF2(alpha + np, np);
      fac *= gammaF2(beta  + np, np + 1 + apb);
      fac /= (double)(beta + np);
...
/**
 Gauss-Radau-Jacobi zeros and weights with end point at \a z=1
...
void zwgrjp(double *z, double *w, int np, double alpha, double beta) 
{
  ...
  if (np == 1) 
	...
	{
	  for(i = 0; i < np; i++)
	    z[i] = -cos((double)(2*i+1)/(double)(2*np-1)*M_PI);
	  for(i = 1; i < (np-1); i++)
	    w[i] = M_PI/(double)(2*np-1);
	  w[0] = w[np-1] = M_PI/2.0/(double)(2*np-1);
	  return;
...
void zwglj(double *z, double *w, int np, double alpha, double beta) 
{
  if (np == 1) 
	...
	{
	  for(i = 0; i < np; i++)
	    z[i] = -cos(M_PI*(double)i/(double)(np-1));
	  for(i = 1; i < (np-1); i++)
	    w[i] = M_PI/(double)(np-1);
	  w[0] = w[np-1] = M_PI/2.0/(double)(np-1);
	  return;
	}
      ...
      fac  = pow(two, apb + 1.0);
      fac *= gammaF2(alpha + np, np);
      fac *= gammaF2(beta  + np, alpha + beta + np + one);
      fac /= (double)(np - 1);
...
/**
 ...
 Compute the derivative matrix, D, and its transpose, Dt,
 associated with the n_th order Lagrangian interpolants through the
 np Gauss-Radau-Jacobi points z such that
 \frac{du}{dz}(z[i]) =  \sum_{j=0}^{np-1} D[i*np+j] u(z[j])
	...
	for (j = 0; j < np; j++) 
	  {
		...
		if (i == 0)
		  ...
		  D[i * np + j] = (alpha - beta + one + (alpha + beta
							 + one) * z[i]) / (two * (one - z[i] * z[i]));
	      }
...
{  
  if (np <= 0) 
	...
	pd[i] *= (1 - z[i]);
      ...
      pd[np-1]  = gammaF2(np + alpha + one, np);
      pd[np-1] /= -1.0*gammaF(alpha + two);
...
/**
   ...
   Compute the derivative matrix, D, and its transpose, Dt,
   associated with the n_th order Lagrangian interpolants through the
   np Gauss-Lobatto-Jacobi points z such that
   \frac{du}{dz}(z[i]) =  \sum_{j=0}^{np-1} D[i*np+j] u(z[j]).
...
{  
  if (np <= 0) 
	...
	pd[i] *= (one - z[i] * z[i]);
      ...
      pd[np-1]  = -two/gammaF(alpha + two);
      pd[np-1] *= gammaF2(np + alpha, np - one);
...
/**
   ...
   the np Gauss-Jacobi points zgj at the arbitrary location z, -1 \leq z \leq 1.
   ...
   \begin{array}{ll}
   \displaystyle
   \frac{P_{np}^{\alpha,\beta}(z)}{[P_{np}^{\alpha,\beta}(z_j)]^\prime
   (z-z_j)} & \mbox{if $z \ne z_j$}\		\
   & \						\
   1 & \mbox{if $z=z_j$}
   \end{array}
...
/**
   ...
   z. This routine assumes zgrj includes the point -1, -1 \leq z \leq 1.
   
   Uses the definition of the Lagrangian interpolant:
   \begin{array}{rcl}
   h_j(z) = \left\{
   \begin{array}{ll}
   \displaystyle
   \frac{(1+z) P_{np-1}^{\alpha,\beta+1}(z)}
...
/**
   Compute the value of the i-th Lagrangian interpolant through the
   np Gauss-Lobatto-Jacobi points zgrj at the arbitrary location z. -1 \leq z \leq 1.
...
/**
   ...
   (including z=-1) to an arbitrary distribution at points zm.
...
/**
   ...
   to an arbitrary distribution at points zm.
	     ...
	     double alpha, double beta) 
{
  ...
  if (n == 0) 
      ...
      for (k = 2; k <= n; ++k) 
	{
	  ...
	  a3 = (two * k + apb - two) * (two * k + apb - one)
	    * (two * k + apb);
...
/**
   ...
   where \Gamma(1/2) = \sqrt(\pi).
*/
double gammaF2(double x, double y) 
{
  ...
  if(x == -0.5 || y == -0.5 || !x || !y || nv <= 1)
    return gammaF(x)/gammaF(y);
...
/**
   Zero determination through the eigenvalues of a tridiagonal
   ...
   where j=n+1\f$ and \f$p_j(z) are the Jacobi (normalized)
...
static void JacZeros(int n, double *a, double alpha, double beta) 
{
  ...
  b[n - 1] = pow(2.0, apb + 1.0) * gammaF(alpha + 1.0) * gammaF(beta + 1.0)
    / gammaF(apbi);
  a[0] = (beta - alpha) / apbi;
  b[0] = sqrt(4.0 * (1.0 + alpha) * (1.0 + beta) / ((apbi + 1.0) * apbi
						    * apbi));
  ...
  for (i = 1; i < n - 1; ++i) 
    {
      ...
      b[i] = sqrt(4.0 * (i + 1) * (i + 1 + alpha) * (i + 1 + beta) * (i + 1
								      + apb) / ((apbi * apbi - 1) * apbi * apbi));
    }
